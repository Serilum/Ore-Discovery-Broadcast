...
public class MiningEvent {
	...
	public static void onBlockBreak(Level level, Player player, BlockPos pos, BlockState state, BlockEntity blockEntity) {
		...
		if (ConfigHandler.hideDeepslateFromName) {
			...
			Identifier blockIdentifier = blockRegistry.getKey(block);
			if (blockIdentifier.toString().contains("deepslate_")) {
				Identifier defaultOreLocation = Identifier.parse(blockIdentifier.toString().replace("deepslate_", ""));
				Optional<Holder.Reference<Block>> defaultOreBlockOptionalReference = blockRegistry.get(defaultOreLocation);
				if (defaultOreBlockOptionalReference.isPresent()) {
					oreName = defaultOreBlockOptionalReference.get().value().getName().getString();
				}
			}
		}
